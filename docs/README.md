# Documentation Organization

This folder contains all project documentation organized by category for easy navigation.

## 📁 Folder Structure

### 📚 Main Documentation Categories

- **[getting-started/](getting-started/)** - Quick start guides and initial setup instructions
- **[features/](features/)** - Feature documentation and user guides
- **[setup-deployment/](setup-deployment/)** - Server setup and deployment guides
- **[authentication/](authentication/)** - Authentication and credential management
- **[database-storage/](database-storage/)** - Database and Supabase storage documentation
- **[photo-image-handling/](photo-image-handling/)** - Photo upload and image processing workflows
- **[compliance/](compliance/)** - Platform compliance and TOS guidelines
- **[platforms/](platforms/)** - Platform integration documentation (eBay, Mercari, etc.)
- **[technical-details/](technical-details/)** - Technical specifications and architecture

## 🔑 Key Files (Located in Root)

These important files remain in the project root for quick access:

- **README.md** - Main project README
- **NON_NEGOTIABLES.md** - Quick reference to all rule files
- **FRONTEND_RULES.md** - Non-negotiable rules for JavaScript, templates, and photo uploads
- **BACKEND_RULES.md** - Non-negotiable rules for database, API, and file handling
- **EXTERNAL_API_RULES.md** - Non-negotiable rules for Gemini, Claude, Supabase, and Mercari APIs
- **/image flow/** - Detailed technical contracts with working code examples

## 📖 How to Use This Documentation

1. **New to the project?** Start with [getting-started/](getting-started/)
2. **Working on code?** Read the rule files first: `FRONTEND_RULES.md`, `BACKEND_RULES.md`, `EXTERNAL_API_RULES.md`
3. **Looking for a specific feature?** Check [features/](features/)
4. **Setting up deployment?** See [setup-deployment/](setup-deployment/)
5. **Need technical details?** Explore [technical-details/](technical-details/)

## 📝 Notes

- Documentation files have been reorganized from the root directory into this structured folder system
- All verbose fix/explanation documents have been consolidated into 3 non-negotiable rule files in the root
- All duplicate and archived documentation has been removed
- For the complete file history, check the git commit log

---

*Last organized: January 2026*
